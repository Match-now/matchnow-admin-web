// src/js/tabs/football-schedule.js 수정 버전
const FootballSchedule = {
        ...
        <div class="football-content">
            ...
            <div class="search-bar">
                ...
                <select id="usage-filter">
                    ...
                    <option value="inactive" selected>미사용중</option>
                </select>
    ...
    attachEventListeners() {
        // 기존 이벤트 리스너들은 유지하되, 새로운 UI에 맞게 수정
        // 검색 관련 이벤트들은 HTML에서 직접 호출로 처리
    },
    ...
    async loadMatches() {
        ...
        try {
            const response = await CONFIG.api.get('/enhanced-football/matches/inplay');
    ...
    createMatchesTable(matches) {
        if (matches.length === 0) {
            return '<div class="error">경기 데이터가 없습니다.</div>';
        }
            ...
            return `
                <tr>
                    ...
                    <td class="datetime-input">
                        <input type="date" value="${matchTime.split(' ')[0] || ''}" onchange="FootballSchedule.updateMatchTime('${match.id}', this, 'date')">
                        <input type="text" value="${matchTime.split(' ')[1] || ''}" onchange="FootballSchedule.updateMatchTime('${match.id}', this, 'time')">
                    </td>
                    <td>
                        <select onchange="FootballSchedule.updateMatchStatus('${match.id}', this.value)">
                    ...
                    <td>
                        ...
                        <div class="time-inputs">
                            ...
                            <input type="text" value="${match.timer?.tm || '0'}">
                        ...
                        <div class="state-content-buttons">
                            <button onclick="FootballSchedule.saveServerTime('${match.id}')">서버시간저장</button>
                            <button onclick="FootballSchedule.saveInputTime('${match.id}')">입력시간저장</button>
                        </div>
                    </td>
                    <td>
                        <div class="team-score">
                            ${match.home?.name || '홈팀'}
                            <input type="text" value="${homeScore || '0'}" onchange="FootballSchedule.updateScore('${match.id}', this.value, 'home')">
                        </div>
                    </td>
                    <td>
                        <div class="team-score">
                            ${match.away?.name || '원정팀'}
                            <input type="text" value="${awayScore || '0'}" onchange="FootballSchedule.updateScore('${match.id}', this.value, 'away')">
                        </div>
                    </td>
                    <td>
                        <button class="broadcast-btn" onclick="FootballSchedule.openBroadcast('${match.home?.name || '홈팀'} vs ${match.away?.name || '원정팀'}', '${match.id}')">중계</button>
                        <button class="broadcast-btn" onclick="FootballSchedule.openLineup('${match.id}')">라인업</button>
                        <button class="broadcast-btn" onclick="FootballSchedule.clearBroadcast('${match.id}')">중계글 지우기</button>
                    </td>
    ...
    toggleStateMode(index) {
        const toggleOptions = document.querySelector(`input[name="state${index}"]:checked`).closest('td').querySelector('.toggle-options select');
        const checkedInput = document.querySelector(`input[name="state${index}"]:checked`);
    ...
    updateMatchTime(matchId, input, type) {
